...
>
> **UPDATE:** I hated working with the old Importing and Exporting system, so I'm rewriting that one too. That means that old exports, be it database, collection or settings, will work, and are automatically upgraded to the newer system. **What will not work is in-place upgrades, I still have no interest upgrading the Database from old to new, so you still definitely need external backups in `.jgvdb` files.**
>
