...
import { collectionManager, galleryElm } from "./globals.js";
...
import { confirmation, downloadURI, uuidtime, uuid, arrayInvertAxis, type UUIDTime } from "./util.js";
...
<br><label for="doSettingsImport">Import Settings?</label><input name="doSettingsImport" type="checkbox" checked>`, ()=>{
    ...
    db: { // Database
        export: async () => {
            galleryElm.collection?.save();
            let conf = {
                type: 0,
                version: jgvdb_local.v.version,
                data: jgvdb_local.f.exportlocalstorage()
            };
            const media = galleryElm.collection!.blobs; // I am sure it exists. TODO: Implement a check?
            let themedia = [];
    ...
    mc: { // Media Collection
        ...
        import: async (conf, files) => {
            ...
            const collection = await collectionManager.newCollection("database");
            collection.rename(conf.data.name);
            ...
            const futureMediaOrder: UUIDTime[] = conf.data.data.map(id => oldIdToNewId.get(id)).filter(v => v !== undefined);
            // Add images
            await jgvdb_local.f.importmedia(files);
            collection.append(...files);
            collection.reorder(futureMediaOrder);
